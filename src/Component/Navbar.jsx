    ...
    <>
      <div className="fixed top-0 w-full bg-white md:bg-white lg:bg-white sm:bg-blue-500 z-50 shadow-2xl">
        <div className="flex justify-between items-center h-14 px-4 sm:px-6 md:px-8">
          ...
          <div className="md:hidden">
            ...
            >
              <svg
                className={`w-6 h-6 transition-transform duration-1000 ${isMenuOpen ? "rotate-90" : "rotate-0"
                  }`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
          ...
          <div className="hidden md:flex md:space-x-4 lg:space-x-8 overflow-x-auto whitespace-nowrap">
            {tabs.map((tab) => (
              ...
              >
                ...
                {tab.name}
                <div
                  className={`absolute bottom-0 left-0 w-full h-0.5 bg-blue-500 transition-transform duration-1000 ${activeTab === tab.name ? "scale-x-100" : "scale-x-0"
                    }`}
                />
              </button>
            ))}
      ...
      {isMenuVisible && (
        <div className="md:hidden fixed inset-0 z-40 bg-white/5 bg-opacity-50 backdrop">
          <div
            className={`mobile-menu-container fixed top-14 left-0 w-64 bg-white rounded-br-2xl shadow-xl transition-all duration-1000 ease-in-out transform ${isAnimating && isMenuOpen
            ...
            <div className="flex flex-col py-4">
              {tabs.map((tab, index) => (
                ...
                >
                  ...
                  <div
                    className={`absolute left-0 top-0 w-1 h-full bg-blue-800 transition-transform duration-1000 ${activeTab === tab.name ? "scale-y-100" : "scale-y-0"
                      }`}
                  />
                </button>
              ))}
